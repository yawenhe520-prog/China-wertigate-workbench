...
function analysisNotice() {
  ...
  if (!connection.configured) return `<div class="notice">尚未配置模型。现在可以核对原文；设置模型后再进行语义分析。${button('配置模型服务', 'data-settings')}</div>`;
  return '';
}
...
async function showSettings() {
  ...
  document.getElementById('modalRoot').innerHTML = `<div class="modal-backdrop"><form class="modal" id="modelSettings"><h2>模型服务设置</h2><p>连接后可分析会议纪要与问题清单。分析时将提取的文档文本发送给所选服务商，其 API 费用按你的服务账户计费。</p>${location.protocol === 'file:' ? '<div class="notice">请先双击“start.command”，再从联网页面配置。</div>' : ''}<label class="field-label">模型服务<select id="provider" class="action-input">${providers.map(provider => `<option value="${esc(provider.id)}" ${connection.provider === provider.id ? 'selected' : ''}>${esc(provider.label)}</option>`).join('')}</select></label><label class="field-label">模型名称<input id="modelName" class="action-input" required value="${esc(connection.model || providers[0].defaultModel)}"/></label><label class="field-label">API Key<input type="password" class="action-input" id="modelKey" autocomplete="off" spellcheck="false" placeholder="${connection.configured ? '已有密钥；留空保留' : '在此输入服务商提供的 API Key'}"/></label><p class="muted">密钥仅保存在本机服务的运行内存中，重启服务后需重新输入；不会写入文档、浏览器存储或工作区备份。可在 <a href="https://platform.deepseek.com" target="_blank" rel="noreferrer">DeepSeek 开放平台</a> 开通 API 服务。</p><div class="analysis-error" id="settingsMessage" role="status"></div><div class="modal-actions"><button type="button" class="btn" id="clearConfig">清除配置</button><button type="button" class="btn" data-close>关闭</button><button type="submit" class="btn primary" ${connection.available ? '' : 'disabled'}>保存并测试连接</button></div></form></div>`;
